...
function addContent(sol, prob) {
    sol = _.map(sol, function (it) {
        var nd = $m(it.to).subtract(1, "minute");
        var r = $m.range(it.from, nd);
        _.map(prob.content, function (t, k) {
            var d = $m(k, "D MMMM YYYY HH:mm");
            if (r.contains(d)) {
...
function produceContentTemplate(sol) {
    var dt = _.groupBy(sol, function (i) {
        var date = $m(i.gDate, "MM/DD/YYYY HH:mm");
        return date.format("D MMMM YYYY HH:mm");
    });
    return _.mapValues(dt, function (i) {
        return {
            contenuto: "",
            type: i[0].tag
        };
...
module.exports = {
    produceStats: produceStats, produceSolution: produceSolution, produceNotes: produceNotes, produceContentTemplate: produceContentTemplate
};
